...
import { answerRelevancyJudgeModel } from "~/models";
import { z } from "zod";
...
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object : answerStatements } = await generateObject({
    model : answerRelevancyJudgeModel,
  ...
  if (answerStatements.statements.length === 0) {
    return {
      score: 0,
      metadata: {
        rational : "No statements found in the submission.",
      },
    };
  }


  const { object : relevancyVerdicts } = await generateObject({
    model : answerRelevancyJudgeModel,
    system: ANSWER_RELEVANCY_AGENT_INSTRUCTIONS,
    prompt: generateEvaluatePrompt({
      input: opts.question,
      statements: answerStatements.statements,
    }),
    schema: z.object({
      verdicts: z.array(
        z.object({
  ...
  const totalScore = relevancyVerdicts.verdicts.reduce((acc, v) => acc + scores[v.verdict], 0);

  const averageScore = totalScore / relevancyVerdicts.verdicts.length;

  return {
    score: averageScore,
    metadata: {
      verdicts: relevancyVerdicts.verdicts,
      statements: answerStatements.statements,
    },
  };
};
...
export const answerRelevancyJudge = createScorer<
  string,
  string,
  string
>({
